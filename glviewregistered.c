...
int mmToRaw(float depthValue) {
	int z = 2047;
	if (depthValue > 0) {
		z = (int) ((1.0/(depthValue/1000.0)) - 3.3309495161)/(-0.0030711016);
		if (z < 0) z = 0;
	}
	return z;
}
...
void keyPressed(unsigned char key, int x, int y)
{
	...
	// Commented out because Fakenect does not have such functions
	// if (key == 'w') {
	// 	freenect_angle++;
...
void depth_cb(freenect_device *dev, void *v_depth, uint32_t timestamp)
{
	...
	pthread_mutex_lock(&gl_backbuf_mutex);
	for (i=0; i<640*480; i++) {
		int rawDepth = mmToRaw(depth[i]);
		if (rawDepth > 0 && rawDepth != 2047) printf("depth: %f, raw = %f\n", depth[i], rawDepth);
		int pval = t_gamma[rawDepth];
		int lb = pval & 0xff;
		switch (pval>>8) {
			case 0:
				depth_mid[3*i+0] = 255;
